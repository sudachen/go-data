...
/*
PostInc increments counter and returns OLD value if old value less then max
*/
func (c *AtomicCounter) PostIncIfLess(max uint64) uint64 {
	for {
		v := atomic.LoadUint64(&c.Value)
		if v < max {
			if atomic.CompareAndSwapUint64(&c.Value, v, v+1) {
...
/*
Inc increments counter and returns NEW value
*/
...
/*
Inc increments counter if current is less then max and returns NEW value
*/
func (c *AtomicCounter) IncIfLess(max uint64) uint64 {
	for {
		v := atomic.LoadUint64(&c.Value)
		if v < max {
			if atomic.CompareAndSwapUint64(&c.Value, v, v+1) {
