...
func Convert(v reflect.Value, na bool, tp reflect.Type) reflect.Value {
	...
	if v.Type() == tp {
		...
		case reflect.Float32:
			return v
		case reflect.Struct:
			if q, ok := v.Interface().(Fixed8); ok {
				return reflect.ValueOf(q.Float32())
			}
		case reflect.Float64:
	...
	} else if tp.Kind() == reflect.Float64 {
		switch v.Kind() {
		case reflect.Float64:
			return v
		case reflect.Struct:
			if q, ok := v.Interface().(Fixed8); ok {
				return reflect.ValueOf(float64(q.Float32()))
			}
		case reflect.Float32:
			return reflect.ValueOf(v.Float())
		case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
			return reflect.ValueOf(float64(v.Uint()))
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
