...
type ColDesc func(string) (string, string, bool)

func Describe(names []string, opts []interface{}) (ColDesc, error) {
	...
	for _, o := range opts {
		if sto, ok := o.(SqlTypeOpt); ok {
			...
			if !exists {
				return nil, errors.Errorf("field %v does not exist in table/query file", v)
			}
		}
	}
