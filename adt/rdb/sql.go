...
func Read(dbc interface{}, opts ...interface{}) (t adt.Table, err error) {
	err = Source(dbc, opts...).Drain(t.Sink())
	return
}

func Write(t adt.Table, dbc interface{}, opts ...interface{}) error {
...
func connectDB(dbc interface{}, opts []interface{}) (db *sql.DB, o []interface{}, err error) {
	...
	if url, ok := dbc.(string); ok {
		...
		err = errors.Errorf("unknown database %v", dbc)
	} else {
...
func Source(dbc interface{}, opts ...interface{}) lazy.Source {
	return func(xs ...interface{}) lazy.Stream {
		...
		return pf(worker, func() lazy.Stream {
			...
			if err != nil {
				lazy.Error(errors.Wrapf(err, "database connection error: %s", err.Error()))
			}
			...
			if err != nil {
				cls.Close()
				return lazy.Error(errors.Wrapf(err, "query error: %s", err.Error()))
			}
			...
			if err != nil {
				cls.Close()
				return lazy.Error(errors.Wrapf(err, "get types error: %s", err.Error()))
			}
			ns, err := rows.Columns()
			if err != nil {
				cls.Close()
				return lazy.Error(errors.Wrapf(err, "get names error: %s", err.Error()))
			}
			...
			factory := adt.SimpleRowFactory{Names: names}
...
func Sink(dbc interface{}, opts ...interface{}) lazy.WorkerFactory {
	...
	if err != nil {
		return lazy.ErrorSink(errors.Errorf("database connection error: %w", err))
	}
	drv := fu.StrOption(Driver(""), opts)
	...
	if err != nil {
		cls.Close()
		return lazy.ErrorSink(errors.Wrapf(err, "query error: %s", err.Error()))
	}

	tx, err := db.Begin()
	if err != nil {
		cls.Close()
		return lazy.ErrorSink(errors.Wrapf(err, "database begin transaction error: %s", err.Error()))
	}
	...
	if fu.Option(ErrorIfExists, opts).Interface().(IfExists_) == DropIfExists {
		...
		if err != nil {
			cls.Close()
			return lazy.ErrorSink(errors.Wrapf(err, "drop table error: %s", err.Error()))
		}
	}
	...
	return lazy.Sink(func(val interface{}, closeErr error) (err error) {
		...
		case *adt.Row:
			if !created {
				...
				if err != nil {
					return errors.Wrapf(err, "create table error: %s", err.Error())
				}
				created = true
			}
		...
		default:
			return errors.Errorf("unsupported value type %v",fu.TypeOf(val))
		}
		return
	})
}
